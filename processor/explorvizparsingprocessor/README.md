...
If an entity can be successfully extracted, information about it is written into the `explorviz.entity.id` and `explorviz.entity.descriptor` attributes for downstream processing. Telemetry for which no entity can be derived is dropped. Additionally, the identifier of the ExplorViz landscape to which visualization information should be matched must be specified by the telemetry agent by writing the `explorviz.token.id` attribute (and the `explorviz.token.secret` value if landscape token validation is enabled).
